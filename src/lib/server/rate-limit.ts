...
export class Throttler<_Key> {
  public timeoutSeconds: string[]; // Store timeout as strings

  private storage = new Map<_Key, ThrottlingCounter>();

  constructor(timeoutSeconds: string[]) {
    this.timeoutSeconds = timeoutSeconds;
  }
  ...
  public consume(key: _Key): boolean {
    ...
    // Parse the current timeout as a number
    const currentTimeout = parseFloat(this.timeoutSeconds[counter.timeout]);
    if (isNaN(currentTimeout)) {
      throw new Error("Invalid timeout value; must be a number string.");
    }

    const allowed = now - counter.updatedAt >= currentTimeout * 1000;
    if (!allowed) {
      return false;
    }
