...
import { updateFloor } from "@/app/admin/floors/actions";
...
const EditFloorForm = ({ floor }: { floor: Floor }) => {
  ...
  const form = useForm<z.infer<typeof createFloorSchema>>({
  ...
  const onSubmit = async (formValues: z.infer<typeof createFloorSchema>) => {
    setMessage(undefined);

    const result = await updateFloor(formValues, floor.id);

    if (Array.isArray(result?.message)) {
      result.message.forEach((issue: ZodIssue) => {
        form.setError(issue.path[0] as keyof CreateFloorSchemaType, {
          type: "custom",
      ...
      <form ref={formRef} onSubmit={form.handleSubmit(onSubmit)}>
        <div className="border-b border-border pb-12">
          ...
          <p className="mt-1 text-sm leading-6 text-muted-foreground mb-8">
            Edit the form below to update the floor
          </p>
