...
import { createAddon } from "@/app/admin/addons/actions";
import { AlertMessage } from "@/components/alert-message";
import { AlertMessageProps } from "@/components/alert-message";
import { createAddonsSchema, CreateAddonsSchemaType } from "../schema";

const CreateAddonsForm = () => {
  ...
  const onSubmit = async (formValues: CreateAddonsSchemaType) => {
    setMessage(undefined);

    const result = await createAddon(formValues);
      ...
      <form ref={formRef} onSubmit={form.handleSubmit(onSubmit)}>
        <div className="border-b border-border pb-12">
          ...
          <p className="mt-1 text-sm leading-6 text-muted-foreground mb-8">
            Fill in the form below to create a new addon
          </p>
