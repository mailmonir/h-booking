...
import { updatePaymentStatus } from "@/app/admin/payment-status/actions";
...
const EditPaymentStatusForm = ({
  ...
  const form = useForm<z.infer<typeof createPaymentStatusSchema>>({
    resolver: zodResolver(createPaymentStatusSchema),
    defaultValues: {
  ...
  const onSubmit = async (
    formValues: z.infer<typeof createPaymentStatusSchema>
  ) => {
    setMessage(undefined);

    const result = await updatePaymentStatus(formValues, paymentStatus.id);

    if (Array.isArray(result?.message)) {
      result.message.forEach((issue: ZodIssue) => {
        form.setError(issue.path[0] as keyof CreatePaymentStatusSchemaType, {
          type: "custom",
      ...
      <form ref={formRef} onSubmit={form.handleSubmit(onSubmit)}>
        <div className="border-b border-border pb-12">
          ...
          <p className="mt-1 text-sm leading-6 text-muted-foreground mb-8">
            Edit the form below to update the payment status
          </p>
          ...
          <div className="mt-10 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
            <div className="sm:col-span-4">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>Payment status name</FormLabel>
                    <FormControl>
                      <Input {...field} />
                    </FormControl>
                    <FormMessage />
